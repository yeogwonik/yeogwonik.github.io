...
1. 모든 적을 섬멸하는 것을 중점으로 모든 적을 섬멸 했을 때 게임에서 승리하게 된다. (result 값이 0이 되면 승리)
2. 적과 플레이어의 거리가 어느 정도 가까워지면 적이 플레이어를 발견하고 추적한다. (distance를 이용해 오브젝트와 오브젝트와의 거리 계산)
3. 적끼리 공격은 통하지 않는다. (Tag를 사용하여 데미지가 들어가는 상대를 분류)
4. 플레이어의 공격은 관통된다. (미정)
